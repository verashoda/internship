...
#    return words_freq[:n]
#
#common_words = get_top_bigrams(df['message'], 20) #change input with the dataframe name
##for word, freq in common_words:
##    print(word, freq)
#df1 = pd.DataFrame(common_words, columns = ['message', 'count'])
#df1.groupby('message').sum()['count'].sort_values(ascending=False).plot(kind='bar')
...
common_words = get_top_trigrams(df['message'], 20) #change input with the dataframe name
#for word, freq in common_words:
#    print(word, freq)
df1 = pd.DataFrame(common_words, columns = ['message', 'count'])
df1.groupby('message').sum()['count'].sort_values(ascending=False).plot(kind='bar')
